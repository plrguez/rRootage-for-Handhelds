...
static int shipMv[8][2] = {
   {0, -256}, {181, -181}, {256, 0}, {181, 181}, {0, 256}, {-181, 181}, {-256,
                                                                         0},
   {-181, -181},
};
...
//  if ( (!laserOnByDefault && (btn & PAD_BUTTON1)) ||
//       (laserOnByDefault && (!(btn & PAD_BUTTON1)))) {
   if ((!settings.laser_on_by_default && (btn & PAD_BUTTON1)) ||
       (settings.laser_on_by_default && (!(btn & PAD_BUTTON1)))) {
      if (status == IN_GAME) {
         addLaser ();
         if (ship.laserCnt == 0) {
            playChunk (0);
   ...
   if ((btn & PAD_BUTTON2) && status == IN_GAME) {
      ...
      case NORMAL_MODE:
         if (bomb > 0
             && (ship.bombCnt <= 0 || ship.bombCnt > BOMB_NEXT_READY_CNT)
             && ship.invCnt < SHIP_INVINCIBLE_CNT_BASE - 30) {
            bomb--;
            bombUsed++;
   ...
   case PSY_MODE:
      ...
      if (ship.grzCnt >= INVINCIBLE_GRZ_NUM) {
         ...
         ship.grzInvCnt = INVINCIBLE_GRZ_CNT;
         for (i = 0; i < 32; i++) {
            addGrazeFrag (ship.pos.x, ship.pos.y, randNS (30000),
                          randNS (30000));
         }
         ship.invCnt = 0;
         playChunk (10);
   ...
   case IKA_MODE:
      if (ship.colorChgCnt > 0) {
         ...
         if (ship.colorChgCnt < COLOR_CHG_CNT / 2) {
            ship.fldWdt =
               (COLOR_CHG_CNT / 2 -
                ship.colorChgCnt) * FLD_WIDTH / (COLOR_CHG_CNT / 2);
         } else {
            ship.fldWdt =
               (ship.colorChgCnt -
                COLOR_CHG_CNT / 2) * FLD_WIDTH / (COLOR_CHG_CNT / 2);
         }
      }
      if (ship.absEng >= ABSENG_CNT) {
         ship.absEng -= ABSENG_CNT;
   ...
   if (sd >= 0) {
      ...
      ship.pos.y += (ship.speed * shipMv[sd][1]) >> 8;
      if (ship.pos.x <
          -FIELD_WIDTH_8 / 2 + SHIP_FIELD_WIDTH * SHIP_SCREEN_EDGE_WIDTH) {
         ship.pos.x =
            -FIELD_WIDTH_8 / 2 + SHIP_FIELD_WIDTH * SHIP_SCREEN_EDGE_WIDTH;
      } else if (ship.pos.x >
                 FIELD_WIDTH_8 / 2 -
                 SHIP_FIELD_WIDTH * SHIP_SCREEN_EDGE_WIDTH) {
         ship.pos.x =
            FIELD_WIDTH_8 / 2 - SHIP_FIELD_WIDTH * SHIP_SCREEN_EDGE_WIDTH;
      }
      if (ship.pos.y <
          -FIELD_HEIGHT_8 / 2 + SHIP_FIELD_WIDTH * SHIP_SCREEN_EDGE_WIDTH) {
         ship.pos.y =
            -FIELD_HEIGHT_8 / 2 + SHIP_FIELD_WIDTH * SHIP_SCREEN_EDGE_WIDTH;
      } else if (ship.pos.y >
                 FIELD_HEIGHT_8 / 2 -
